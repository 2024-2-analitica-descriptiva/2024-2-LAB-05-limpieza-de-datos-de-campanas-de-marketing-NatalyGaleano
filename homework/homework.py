...
def clean_campaign_data():
    ...
    clientes_df = dataframe[['client_id', 'age', 'job', 'marital', 'education', 'credit_default', 'mortgage']].copy()

    clientes_df['job'] = clientes_df ['job'].str.replace(r"[.]", "", regex=True).str.replace(r"[-]", "_", regex=True)
    clientes_df['education'] = clientes_df ['education'].str.replace(r"[.]", "_", regex=True).replace("unknown", pd.NA)
    clientes_df['credit_default'] = clientes_df ['credit_default'].apply(lambda x: 1 if x == "yes" else 0)
    clientes_df['mortgage'] = clientes_df['mortgage'].apply(lambda x: 1 if x == "yes" else 0)

    save_csv_in_directory(clientes_df , output_path, "client.csv")
...
def read_zip_and_concat(input_path):
    ...
    data_completa = []  # Lista para almacenar los DataFrames
            ...
            with zipfile.ZipFile(zip_path, 'r') as zip_file:
                ...
                for csv_file in csv_files:
                    ...
                    with zip_file.open(csv_file) as file:
                        ...
                        data_completa.append(df)
    dataframe_df = pd.concat(data_completa, ignore_index=True)
